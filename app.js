...
import { buildMeaning } from "./meaning.js?v=20261045";
import {
  buildLights,
  attachImpulse,
  memberAnchorScore,
  ...
  try {
    ...
    el.innerHTML = favor.items
      .map((it) => {
        ...
        const isBtc = it.id === "crypto" && !!price;
        const aria = `aria-label="${escapeHtml(title)}, ${word}${
          price ? `, ${isBtc ? `Bitcoin ${price}` : price}` : ""
        ...
        return `<button type="button" class="favor-cell favor-ust" data-favor-id="${escapeHtml(
          ...
          <span class="favor-curve">
            ...
            ${
              price
                ? isBtc
                  ? `<span class="favor-tenor favor-spot-px favor-spot-btc">₿ ${escapeHtml(
                      price
                    )}</span>`
                  : `<span class="favor-tenor favor-spot-px favor-spot-gold"><span class="favor-price">${escapeHtml(
                      price
                    )}</span></span>`
                : ""
            }
